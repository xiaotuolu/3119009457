...
def assume_role_with_saml(role, principle, saml, count, display_name, region):
    ...
    try:
        cred = stsclient.assume_role_with_saml(
            RoleArn=role, PrincipalArn=principle, SAMLAssertion=saml, DurationSeconds=43200)
    except ClientError as e12:
        try:
            cred = stsclient.assume_role_with_saml(
                RoleArn=role, PrincipalArn=principle, SAMLAssertion=saml, DurationSeconds=28800)
        except ClientError as e8:
            try:
                cred = stsclient.assume_role_with_saml(
                    RoleArn=role, PrincipalArn=principle, SAMLAssertion=saml, DurationSeconds=14400)
            except ClientError as e4:
                try:
                    cred = stsclient.assume_role_with_saml(
                        RoleArn=role, PrincipalArn=principle, SAMLAssertion=saml, DurationSeconds=7200)
