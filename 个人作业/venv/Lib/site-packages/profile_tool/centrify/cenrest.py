...
#    logging.info("And body : " + str(body))
    ...
    try:
        ...
        print(Fore.RED + 'Error in calling ' +
              endpoint + ' - Please refer logs. ')
...
def call_rest_post_redirect(endpoint, method, body, headers, certpath, proxy, allow_redirects=True):
    ...
    logging.info("Method : " + method + " Request Body : " + str(body) +
                 " Headers : " + str(headers) + " Proxy : " + str(proxy))
    logging.info("Calling " + endpoint + " with headers : " +
                 str(headers) + " and data : " + str(body))
    response = requests.post(endpoint, headers=headers,
                             verify=certpath, proxies=proxy, data=body)
    logging.info("Received Response : " + response.text)
    return response
